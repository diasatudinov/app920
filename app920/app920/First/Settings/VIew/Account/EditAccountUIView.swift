            ...
            VStack(spacing: 20) {
                ...
                Button {
                    if !name.isEmpty && !age.isEmpty && !gender.isEmpty && !description.isEmpty {
                        if let image = selectedImage {
                            let account = Account(imageData: image.jpegData(compressionQuality: 1.0), name: name, age: age, gender: gender, description: description)
                            viewModel.updateAccount(account: account)
                        } else {
                            let account = Account(name: name, age: age, gender: gender, description: description)
                            viewModel.updateAccount(account: account)
                        }
...
#Preview {
    EditAccountUIView(viewModel: SettingsViewModel(), account: .constant(Account(imageData: UIImage(named: "notifications920")?.jpegData(compressionQuality: 1.0), name: "Nina", age: "52 Age", gender: "Female", description: "I love to travel")))
}
